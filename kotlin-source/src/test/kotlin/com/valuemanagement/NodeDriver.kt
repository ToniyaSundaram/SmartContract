...
fun main(args: Array<String>) {
    ...
    driver(isDebug = true) {
        ...
        val (nodeA, nodeB, nodeC,nodeD,nodeE) = listOf(
                startNode(providedName = CordaX500Name("InitiatorCognizant", "London", "GB"),rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("AuthorizerCognizant", "New York", "US"), rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("OperationalCustomer", "Paris", "FR"), rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("TacticalCustomer", "Paris", "FR"), rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("StrategicCognizant", "London", "GB"), rpcUsers = listOf(user))
                ).map { it.getOrThrow() }
