...
import com.valuemanagement.flow.AuthorizeValueTransactionFlow.Acceptor
...
object AuthorizeValueTransactionFlow {
    @InitiatingFlow
    @StartableByRPC
    class ValueContractAuthorizer(val valueContractTransactionVO : ValueContractTransactionVO,
                                  val intiatorCognizant: Party,
        ...
        override fun call(): SignedTransaction {
            ...
            val valueContractTransactionState = ValueContractTransactionState( valueContractTransactionVO, authorizerCognizant, listOf(operationalCustomer, intiatorCognizant,counterParties[0],counterParties[1],counterParties[2]));
            valueContractTransactionVO.valueContractTransactionId = valueContractTransactionState.linearId.id
            val txCommand = Command(ValueTransactionContract.Commands.Update(), valueContractTransactionState.participants.map { it.owningKey })
            ...
            val intiatorcognizant = initiateFlow(intiatorCognizant)
            ...
            val fullySignedTx = subFlow(CollectSignaturesFlow(partSignedTx, setOf(operationalcustomer, intiatorcognizant,strategiccognizant,strategiccustomer,tacticalcustomer), GATHERING_SIGS.childProgressTracker()))
